...
def get_common_message_properties(message):
    ...
    properties = {
        ...
        "recipient_email": message.transport_headers,
        "date": message.delivery_time.isoformat() if message.delivery_time else None,
        "body": message.plain_text_body.decode('utf-8', errors='ignore') if message.plain_text_body else None,
        # "folder_path": None, "message_id": message.internet_message_id, "headers": message.transport_headers,
        "attachments": [],
        'type': 'message'
    }
...
def pst_to_jsonl(pst_file_path: str, jsonl_file_path: str) -> bool:
    ...
    """
    print(f"Converting PST to JSONL: {pst_file_path}")
    ...
    try:
        ...
        with open(jsonl_file_path, 'w', encoding='utf-8') as outfile:
            ...
            if root_folder.get_number_of_sub_folders() == 0:
                logging.warning(
                    f"PST file '{pst_file_path}' root folder has no subfolders. Attempting to process root folder itself if it contains messages.")
        ...
        raise Exception(
            f"Error processing PST file with pypff: {e}. Please ensure libpff is correctly installed and the PST file is valid.")
